...
//--------------------------------------------------------------
void AudioAnalyze::plot(vector<float>& buffer, float scale, float offset) {
    ofNoFill();
    int n = buffer.size();
...
//--------------------------------------------------------------
float powFreq(float i) { return powf(i, 3); }


//--------------------------------------------------------------
void AudioAnalyze::audioReceived(float* input, int bufferSize, int nChannels) {
    if (mode == MIC) {
        ...
        for (int i = 0; i < bufferSize; i++) audioBuffer[i] = ofRandom(-1, 1);
    } else if (mode == SINE) {
        for (int i = 0; i < bufferSize; i++) audioBuffer[i] = sinf(PI * i * ofGetMouseX() / appWidth);
    }
...
//--------------------------------------------------------------
void AudioAnalyze::keyEvent(int key){
    switch (key) {
        case 'm':
            mode = MIC;
            break;
